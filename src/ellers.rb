...
def row2verilog(row, width, row_count, last=false)
  s = "      " + (last ? "default: odata <= 'b" : "'d#{row_count-1}:     odata <= 'b")
...
puts "// Autogenerated by: ellers.rb"
puts "module maze_rom_#{width}_#{height} #("
puts "     parameter ADDR_WIDTH = #{Math.log2(height).ceil},"
puts "     parameter DATA_WIDTH = #{width}"
puts ")("
puts "    input clk,"
puts "    input [ADDR_WIDTH-1:0] addr,"
puts "    output reg [DATA_WIDTH-1:0] odata"
puts ");"
puts "  always @(posedge clk) begin"
puts "    case (addr)"
puts "      //" + " " * (width * 2 + 23) + "_" * (width * 2 - 1)

while spinning
  state, row = step(state)
  row_count += 1
  puts row2verilog(row, width, row_count)
...
puts row2verilog(row, width, row_count, true)
