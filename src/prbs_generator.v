...
localparam TYPE_PRBS7  = 0;
localparam TYPE_PRBS9  = 1;
localparam TYPE_PRBS10 = 2;
localparam TYPE_PRBS11 = 3;
localparam TYPE_PRBS15 = 4;
localparam TYPE_PRBS20 = 5;
localparam TYPE_PRBS23 = 6;
localparam TYPE_PRBS29 = 7;
localparam TYPE_PRBS31 = 8;
...
//===================================================
generate
    if(TYPE == TYPE_PRBS7) begin : IMPL_PRBS7
        always @(posedge clock)begin
            if (init) out <= 32'hfe041851;
            else out <= prbs7(out);
        end
    end else if(TYPE == TYPE_PRBS9) begin : IMPL_PRBS9
        always @(posedge clock)begin
            if (init) out <= 32'hff83df17;
            else out <= prbs9(out);
        end
    end else if(TYPE == TYPE_PRBS10) begin : IMPL_PRBS10
        always @(posedge clock)begin
            if (init) out <= 32'hffc070fd;
            else out <= prbs10(out);
        end
    end else if(TYPE == TYPE_PRBS11) begin : IMPL_PRBS11
        always @(posedge clock)begin
            if (init) out <= 32'hffe00c07;
            else out <= prbs11(out);
        end
    end else if(TYPE == TYPE_PRBS15) begin : IMPL_PRBS15
        always @(posedge clock)begin
            if (init) out <= 32'hfffe0004;
            else out <= prbs15(out);
        end
    end else if(TYPE == TYPE_PRBS20) begin : IMPL_PRBS20
        always @(posedge clock)begin
            if (init) out <= 32'hfffff000;
            else out <= prbs20(out);
        end
    end else if(TYPE == TYPE_PRBS23) begin : IMPL_PRBS23
        always @(posedge clock)begin
            if (init) out <= 32'hfffffe00;
            else out <= prbs23(out);
        end
    end else if(TYPE == TYPE_PRBS29) begin : IMPL_PRBS29
        always @(posedge clock)begin
            if (init) out <= 32'hfffffff8;
            else out <= prbs29(out);
        end
    end else /*if(TYPE == TYPE_PRBS31)*/ begin : IMPL_PRBS29
        always @(posedge clock)begin
            if (init) out <= 32'hfffffffe;
            else out <= prbs31(out);
        end
    end
endgenerate
